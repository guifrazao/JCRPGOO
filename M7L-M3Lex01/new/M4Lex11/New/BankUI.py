...
from AccountOperationsExtended import AccountOperationsExtended

class BankUI:
    def __init__(self, bank):
        self.bank = bank
        self.operations = AccountOperationsExtended()
    ...
    def pay_bill(self):
        ...
        account = self.bank.find_account(name, pin)
        if account:
            amount = float(input("Digite o valor da conta a ser paga: R$"))
            self.operations.pay_bill(account, amount)
        else:
            print("Conta não encontrada.")
