...
def mostrar_detalhes_employee(employee):
    try:
        ...
        print(f"Salário Líquido: ${employee.calculateSalary(bonus)}\n")
    except AttributeError:
...
def mostrar_detalhes_administrator(administrator):
    try:
        bonus = administrator.get_subsistenceAllowance()
        ...
        print(f"Salário Líquido (com ajudas de custo): ${administrator.calculateSalary(bonus)}\n")
    except AttributeError:
...
def mostrar_detalhes_factory_worker(factory_worker):
    try:
        bonus = factory_worker.get_valueProduction() * (factory_worker.get_commission() / 100)
        ...
        print(f"Salário Líquido (com comissão): ${factory_worker.calculateSalary(bonus)}\n")
    except AttributeError:
...
def mostrar_detalhes_seller(seller):
    try:
        bonus = seller.get_valueSales() * (seller.get_commission() / 100)
        ...
        print(f"Salário Líquido (com comissão): ${seller.calculateSalary(bonus)}\n")
    except AttributeError:
