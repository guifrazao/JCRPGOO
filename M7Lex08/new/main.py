...
def getEmployeeData():
    try:
        ...
        salary = float(input("Employee's monthly salary: $")) 
        employee_type = int(input("Employee type (1 - Normal employee, 2 - Intern): "))

        if employee_type == 1:
            return Employee(name, surname, salary)
        elif employee_type == 2:
            return Intern(name, surname, salary)
        else:
            raise Exception("Employee type does not exist")
